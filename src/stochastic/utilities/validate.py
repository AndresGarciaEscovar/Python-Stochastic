...
def validate_dictionary(
    ...
    """
        Validates that the given dictionary is the same as the expected
        dictionary
    ...
    def recursive_(base_: dict, current_: dict, path_: str) -> None:
        """
            Recursively checks if the current dictionary is the same as the
            dictionary of the base dictionary; including the types.

            :param base_: The base dictionary.

            :param current_: The dictionary to be validated.

            :param path_: The path to the current dictionary level.
        """
        # Local variables.
        nonlocal message
        ...
        # The keys of the current dictionary are not the same.
        keys_base_: set = set(base_.keys())
        keys_current_: set = set(current_.keys())

        if keys_base_ != keys_current_:
            excess: set = keys_current_ - keys_base_
            missing: set = keys_base_ - keys_current_

            message += (
                f"The keys of the \"{path_}\" dictionary are not the same as "
                f"that of the expected keys. Expected: {keys_base_}, current: "
                f"{keys_current_ or '{}'}, excess: {excess or '{}'}, missing: "
                f"{missing or '{}'}."
            )

        # Check the remaining keys.
        for key_ in keys_base_.intersection(keys_current_):
            # Recursive step.
            tkey_: Any = f"\"{key_}\"" if isinstance(key_, str) else key_
            recursive_(base_[key_], current_[key_], f"{path_}.{tkey_}")
