...
class RSA2D(metaclass=ABCMeta):
    ...
    def attempts_successful(self) -> int:
        """
            Returns the number of successful attempts to place a particle in
            the lattice.
    ...
    def attempts_successful(self) -> None:
        ...
        raise PermissionError(
            "The attempts_successful variable must not be deleted."
        )
    ...
    def lattice(self, _) -> None:
        ...
        """
        self.__lattice = [
            [RSA2D.EMPTY for _ in range(self.dimensions[1])]
    ...
    def random_generator(self) -> None:
        ...
        """
        raise PermissionError(
            "The random_generator variable must not be deleted."
        )
    ...
    def repetitions(self) -> int:
        """
            ...
            :return: The number of times that the simulation must be averaged
    ...
    def repetitions(self, repetitions: int) -> None:
        """
            Sets the number of times that the simulation must be averaged to
            get statistically significant results.

            :param repetitions: The number of times that the simulation must
             be averaged to get statistically significant results. If the
             number is negative, the absolute value will be taken.
        """
    ...
    def results_file(self) -> str:
        """
            ...
            :return: The name of the file where the simulation results are to
             be saved.
        """
    ...
    def statistics_table(self) -> None:
        ...
        """
        raise PermissionError(
            "The statistics_table variable must not be deleted."
        )
    ...
    def get_coverage(self):
        ...
        return sum(
            1 for i, j in iterator if not self.lattice[i][j] == RSA2D.EMPTY
        )
    ...
    def normalize_site(self, site: tuple) -> tuple:
        """
            Given an scalar integer site, returns the site in the lattice, as
            it would correspond to the periodicity.
        ...
        site_ = tuple(
            get_periodic(site[i], self.dimensions[i])
            if self.periodic[i] else site[i]
            for i in range(2)
        )
    ...
    def print_results(self) -> None:
        ...
        def get_colum_widths(header0, statistics_table0):
            """
                ...
                :return: The values with the widths of the columns in the
                 proper format.
            """
            ...
            for statistic0 in statistics_table0:
                for i0, _ in enumerate(statistic0):
                    if i0 > 0:
                        columns_width0[i0] = max(
                            columns_width0[i0], len(f"{statistic0[i0]:0.10f}")
                        )
                        continue

                    columns_width0[i0] = max(
                        columns_width0[i0], len(f"{statistic0[i0]:0.3f}")
                    )
        ...
        with open(self.results_file, encoding="utf-8", mode="w") as fl:
            fl.write(preheader + "\n")
    ...
    def reset_simulation_variables(self) -> None:
        """
            Resets the variables for a single simulation to their initial
            state.
        """
        # Number of dimensions.
        dims = [range(dimension) for dimension in self.dimensions]

        # Reset the lattice.
        for i, j in itertools.product(*dims):
    ...
    def run_simulation_single(self, number: int) -> None:
        ...
        while elapsed_time < self.maximum_time:
            ...
            ratio = (10 * self.attempts) % self.get_number_of_cells()

            if number == 0 and ratio == 0:
    ...
    def save_lattice(self, initial=True, last=False) -> None:
        ...
        header.extend([
            f"({indexes[0]}x{indexes[1]})" for indexes in product(*dims)
        ])
        header = ",".join(header)

        # Always open in append mode.
        with open(self.lattice_file, encoding="utf-8", mode="a") as fl:
            if initial:
            ...
            tmp_stats = ",".join([
                f"{stat:0.3f}" if i == 0 else f"{stat:1}"
                for i, stat in enumerate(tmp_stats)
            ])
